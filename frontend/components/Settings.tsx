...
export default function Settings() {
    ...
    async function handleDeleteAllTimers() {
        if (confirm("Are you sure you want to delete all timers? This action cannot be undone.")) {
            try {
                ...
                console.error(e);
                alert('Failed to delete timers. Please try again.');
            }
        }
    }
    ...
    useEffect(() => {
        (async () => {
            try { await loadMeetingTimerSettings(); } catch (e) { console.error(e); }
        })();
    }, []);
            ...
            <div className="flex-1 p-4 space-y-6">
                ...
                <fieldset className="rounded-2xl border border-gray-200 px-4 py-2">
                    ...
                    <div className="my-2 grid grid-cols-2 gap-3 sm:max-w-md">
                        <label className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-3 ${visibility === "me" ? "border-sky-900 ring-2 ring-sky-400/50" : "border-gray-200 hover:border-gray-300"
                            }`}>
                            <input
                                type="radio"
                                name="visibility"
                        ...
                        <label className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-3 ${visibility === "everyone" ? "border-sky-900 ring-2 ring-sky-400/50" : "border-gray-200 hover:border-gray-300"
                            }`}>
                            <input
                                type="radio"
                                name="visibility"
