...
export default function NudgeStatsPanel() {
  ...
  const moveAlongTotal = useNudgeStore((s) => s.moveAlongTotal);
  const speakUpTotal = useNudgeStore((s) => s.speakUpTotal);
    ...
    <div className="flex flex-col gap-2 p-4 pt-0">
      <div className="flex flex-row gap-2 text-center">
        <div className="w-[50%] bg-green-900/85 text-white p-4 mt-3 rounded-lg shadow-md">
          ...
          <h1 className="text-2xl font-bold text-center">{moveAlongTotal || moveAlong}</h1>
        </div>
        <div className="w-[50%] bg-sky-900/85 text-white p-4 mt-3 rounded-lg shadow-md">
          ...
          <h1 className="text-2xl font-bold text-center">{speakUpTotal || speakUp}</h1>
        </div>
      </div>
    </div>
  );
